...
export default function subjectList (prevState = initSubjectList, action) {
  switch (action.type) {
    case GET_SUBJECT_LIST:
      // 为了实现展示二级课程分类，需要items中每一个数据添加children属性
      // 有了children属性，每个数据就会有可展开按钮了
    ...
    case GET_SECSUBJECT_LIST:
      // 把二级分类，添加到对应的一级分类数据children属性上面
      /*{
        "total": 1,
        "items": [
            {
      ...
      // 1. 获取一级分类的id(需要先知道要把这个数据添加到那个一级里面)
      // 如果没有二级分类数据，不执行后面代码
    ...
    case UPDATE_SUBJECT:
      // 通过prevstate,利用传过来的id，找到要修改的对应数据，修改title
      // 1. 遍历 prevState 是个对象
